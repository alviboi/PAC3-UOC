...
Donada la naturalessa de github el dipòsit pot ser públic o privat, en aquest cas hem decidit fer-lo públic, però podriem haver gestionat el projecte per a que fos provat i estigués compartit únicament amb el professor.\\
...
Una vegada creada la còpia al nostre ordinador per a afegir arxius, en aquest cas els arxius filtres.c i filtres.h que es troben al directori libsrc, per tant hem de tenir en compte que en primer lloc hem de afegir-los al nostre repositori amb el commandament:
\begin{center}
\emph{git add ./libsrc}
\end{center}
Això no significa que els arxius estiguen al nostre depòsit haurem d'afegir-los, amb un comentari. Pit resultar redundant, però d'aquesta manera pots afegir diversos arxius i posteriorment guardar-los tots de cop al dipòsit amb el commandament:
\begin{center}
...
\end{center}
De manera que sempre tindrem accesible el nostre projecte des de qualsevol ordinador. Podem veure com quedaria la pàgina del nostre projecte a github\footnote{Podem veure que els arxius que tenim són de les diferents preguntes que hem anat tractant, no només d'aquesta}.\\
...
\/*/
\end{shaded}
Com podem veure el que hem fer és utilitzar les etiquetes doxygen que ens permeten especificar què esem exlicant en cada punt, així per exemple:
\begin{center}
\begin{tabular}{ | c | c|}
  \hline
...
\end{tabular}
\end{center}
De manera semblant en cadascuna de les funcions fem una xicoteta explació de quin és el seu propòsit tot especificant.
\begin{shaded}
/**\\
...
Hem de recordar que tot i que la documentació ha sigut generada dins de la carperta, dins del nostre dipòsit, fins que no fem un git add i deprés un git commit... els arxius no es trobaran al dipòsit.
